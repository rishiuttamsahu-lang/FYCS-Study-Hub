...
const BrainCircuitIcon = forwardRef(
  ({ size = 24, color = "currentColor", strokeWidth = 2, className = "" }, ref) => {
    ...
    const startAnimation = useCallback(() => {
      // 1. Brain outline subtle pulse (Continuous)
      animate(
        ".brain-outline",
        { opacity: [1, 0.6, 1] },
      ...
      // 2. 🚨 Circuit lines (Front-Reverse-Front "Yoyo" Loop) 🚨
      animate(
        ".circuit-line",
        ...
        { 
          duration: 1.5, 
          repeat: Infinity, 
          repeatType: "reverse", // Ye line draw aur un-draw karegi
          ease: "easeInOut" 
        },
      );

      // 3. Synaptic terminals pulsing in sequence
      const terminals = [
        ".terminal-1",
        ".terminal-2",
        ".terminal-3",
      ...
      terminals.forEach((selector, index) => {
        animate(
          selector,
          { scale: [1, 1.4], opacity: [0.3, 1] },
          {
            duration: 1,
            delay: index * 0.2, // Staggered delay ek-ek karke jalne ke liye
            repeat: Infinity,
            repeatType: "reverse", // Ye dots ko chamkayega aur fade karega
            ease: "easeInOut",
          },
        );
      });
    ...
    // 🚨 MAGIC TRIGGER: Bina hover ke load hote hi animation start 🚨
    useEffect(() => {
      startAnimation();
    }, [startAnimation]);
